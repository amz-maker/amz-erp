...
// =================================================================
export const __CONTROLLER__NAME__ = makeFarestFrame<ApiInput, ApiOutput>(
    'Get-query', 
    async (input, headers) => 
    {
        ...
        const AddWhere = <Input>(bucket: string[], input: Input, key: string, keyName: keyof ApiInput & string, operator: '=' | '<' | '>' | 'LIKE', tabSize:number = 6) => {
            const valPart = operator === 'LIKE' ? `%${input[keyName]}%` : `${input[keyName]}`;
            if(key === keyName) {
                bucket.push(`${' '.repeat(tabSize)}${StringUtil.camelToLargeSnake(key)} ${operator} '${valPart}'`);
            }
        };
        ...
        if(keys.length > 0) {
            ...
            for(const key of keys) {
                AddWhere(conditions, input, key, 'ctrctStartDt', '>');
                AddWhere(conditions, input, key, 'ctrctEndDt', '<');
                AddWhere(conditions, input, key, 'ctrctCompn', 'LIKE');
                AddWhere(conditions, input, key, 'orderCompn', 'LIKE');
                AddWhere(conditions, input, key, 'prjctNm', 'LIKE');
                AddWhere(conditions, input, key, 'payGbCd', '=');
                AddWhere(conditions, input, key, 'ctrctTypeCd', '=');
            }
